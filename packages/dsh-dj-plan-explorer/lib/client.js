...
`;
    ...
    function PlanCard({planRef,t,onAction}){
    ...
    function mountLauncher(t){
      ...
      const button=document.createElement("button");button.type="button";button.className="djPlanLauncher";button.textContent=t("plan");button.hidden=true;button.addEventListener("click",()=>latestPlan&&openPage({kind:"plan",planRef:latestPlan}));
      const place=()=>{button.hidden=!latestPlan;const lists=Array.from(document.querySelectorAll('[role="tablist"]'));const list=lists.find(node=>/轨迹|Trajectory/i.test(node.textContent||""));if(list&&!list.contains(button))list.appendChild(button)};
    ...
    function AuxiliaryHost({width,maximized,t,operatorT,close,toggleMaximized,open}){
      ...
      React.useEffect(()=>{const onOpen=event=>{const next=event.detail;if(next?.kind==="plan"&&next.planRef)registerPlan(next.planRef);setPage(next);open()};window.addEventListener("dsh-dj-open-auxiliary",onOpen);return()=>window.removeEventListener("dsh-dj-open-auxiliary",onOpen)},[open]);
